...
fn preprocessor_maps_high_byte_glyphs_to_p8_identifiers() {
    // 0x80 is a P8SCII glyph that's not a button shorthand — should become
    // a stable `_p8_80` identifier so glyph variable names (used as keys
    // like `t={\u{80}=1}` in BBS carts) still parse.
    let src = "x=\u{80}";
    let out = preprocessor::preprocess(src);
    assert!(out.contains("_p8_80"), "expected _p8_80 in {out:?}");
}
...
fn fixture_hello_p8_loads_clean() {
    ...
    assert!(cart.lua_code.contains("function _init"));
}
