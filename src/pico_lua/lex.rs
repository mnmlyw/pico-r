...
pub enum Tok {
    ...
    Eq, NotEq, Lt, Gt, Le, Ge, Assign,
    LParen, RParen, LBrace, RBrace, LBracket, RBracket,
    DoubleColon, Semi, Colon, Comma, Dot, Concat, Vararg,
    // End of file
    Eof,
}
...
impl<'a> Lexer<'a> {
    ...
    fn try_long_open(&mut self) -> Option<usize> {
        ...
        let start = self.pos;
        if self.src[start] != b'[' { return None; }
        let mut p = start + 1;
        let mut level = 0;
        while p < self.src.len() && self.src[p] == b'=' { level += 1; p += 1; }
    ...
    fn skip_long_bracket(&mut self, level: usize) -> Result<(), LexError> {
        ...
        loop {
            ...
            if self.src[self.pos] == b']' {
                ...
                let mut count = 0;
                while p < self.src.len() && self.src[p] == b'=' && count < level { count += 1; p += 1; }
    ...
    fn read_long_string(&mut self, level: usize) -> Result<Vec<u8>, LexError> {
        ...
        if self.pos < self.src.len() && self.src[self.pos] == b'\n' {
            self.pos += 1;
            self.line += 1;
        } else if self.pos + 1 < self.src.len() && self.src[self.pos] == b'\r' && self.src[self.pos + 1] == b'\n' {
            self.pos += 2;
            self.line += 1;
        }
        loop {
    ...
    fn next_tok(&mut self) -> Result<Tok, LexError> {
        ...
        match c {
            ...
            b'/' => { self.pos += 1; Ok(Tok::Slash) }
            b'%' => { self.pos += 1; Ok(Tok::Percent) }
            b'^' => { self.pos += 1; Ok(Tok::Caret) }
            b'#' => { self.pos += 1; Ok(Tok::Hash) }
            ...
            b'[' => {
                ...
                self.pos += 1; Ok(Tok::LBracket)
            }
            b']' => { self.pos += 1; Ok(Tok::RBracket) }
            b'=' => {
                if self.peek_eq(1, b'=') { self.pos += 2; Ok(Tok::Eq) }
                else { self.pos += 1; Ok(Tok::Assign) }
            }
            b'~' => {
                if self.peek_eq(1, b'=') { self.pos += 2; Ok(Tok::NotEq) }
                else { Err(self.err("expected '=' after '~'")) }
            }
            b'<' => {
                if self.peek_eq(1, b'=') { self.pos += 2; Ok(Tok::Le) }
                else { self.pos += 1; Ok(Tok::Lt) }
            }
            b'>' => {
                if self.peek_eq(1, b'=') { self.pos += 2; Ok(Tok::Ge) }
                else { self.pos += 1; Ok(Tok::Gt) }
            }
            b':' => {
                if self.peek_eq(1, b':') { self.pos += 2; Ok(Tok::DoubleColon) }
                else { self.pos += 1; Ok(Tok::Colon) }
            }
            b'.' => {
                if self.peek_eq(1, b'.') {
                    if self.peek_eq(2, b'.') { self.pos += 3; Ok(Tok::Vararg) }
                    else { self.pos += 2; Ok(Tok::Concat) }
            ...
            if c == b'\\' {
                ...
                match esc {
                    ...
                    b'x' => {
                        ...
                        if n == 0 { return Err(self.err("malformed \\x escape")); }
                        out.push(v as u8);
                    }
                    b'z' => {
                        self.pos += 1;
                    ...
                    b'0'..=b'9' => {
                        let mut v: u32 = 0;
                        let mut n = 0;
                        while n < 3 && self.pos < self.src.len() && self.src[self.pos].is_ascii_digit() {
                            v = v * 10 + (self.src[self.pos] - b'0') as u32;
                            self.pos += 1; n += 1;
                        }
                        if v > 255 { return Err(self.err("decimal escape > 255")); }
                        out.push(v as u8);
                    }
    ...
    fn read_number(&mut self) -> Result<Tok, LexError> {
            ...
            && (self.src[self.pos + 1] == b'x' || self.src[self.pos + 1] == b'X')
        {
            is_hex = true;
            self.pos += 2;
            while self.pos < self.src.len() && is_hex_byte(self.src[self.pos]) { self.pos += 1; }
            if self.pos < self.src.len() && self.src[self.pos] == b'.' {
                self.pos += 1;
                while self.pos < self.src.len() && is_hex_byte(self.src[self.pos]) { self.pos += 1; }
            }
    ...
    fn err_byte(&self, msg: &str, b: u8) -> LexError {
        LexError { msg: format!("{} (byte 0x{:02x})", msg, b), line: self.line }
    }
}

fn is_hex(b: u8) -> bool { is_hex_byte(b) }
fn is_hex_byte(b: u8) -> bool {
    b.is_ascii_digit() || (b >= b'a' && b <= b'f') || (b >= b'A' && b <= b'F')
}
fn hex_val(b: u8) -> u8 {
    match b { b'0'..=b'9' => b - b'0', b'a'..=b'f' => b - b'a' + 10, _ => b - b'A' + 10 }
}
...
fn parse_hex_float(s: &str) -> Option<f64> {
    ...
    while i < bytes.len() && is_hex_byte(bytes[i]) {
        int_part = int_part.checked_mul(16)?.checked_add(hex_val(bytes[i]) as u64)?;
        i += 1;
    }
    ...
    if i < bytes.len() && (bytes[i] == b'p' || bytes[i] == b'P') {
        i += 1;
        let neg = if i < bytes.len() && bytes[i] == b'-' { i += 1; true }
